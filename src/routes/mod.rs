...
pub fn init(cfg: &mut ServiceConfig) {
    ...
    // hosted mode can use /api/settings/xai-oauth/callback on the main app
    // domain. Both are matched to a pending Settings login by OAuth `state`.
    cfg.route("/callback", web::get().to(settings::xai_oauth_callback));
    cfg.route(
        "/api/settings/xai-oauth/callback",
        web::get().to(settings::xai_oauth_callback),
    );
